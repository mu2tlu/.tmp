...
void Channel::setTopic(std::string topic, Client* user)
{
	...
	{
		user->sendMessage(":" + user->getClientIp() + " 482 " + user->getNickname() + " " + getChannelName() + " :You're not channel operator\r\n");
		return;
	}
	if (topic.empty())
	{
		user->sendMessage(":" + user->getClientIp() + " 331 " + user->getNickname() + " " + getChannelName() + " :No topic is set\r\n");
		return;
	}
	this->_topic.erase();
	this->_topic = top_;
