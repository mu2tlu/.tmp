...
void joinChannel(Client* client, const std::string& channelName, const std::vector<std::string>& commandParts, Server* srv)
{
    ...
    client->sendReply(RPL_NAMREPLY(client->getPrefix(), channel->getChannelName(), nickList));
    std::cout << RPL_NAMREPLY(client->getPrefix(), channel->getChannelName(), nickList) << std::endl;
...
// 	    	nickList += *it + " ";
// 	    }
//         std::string serverName = srv->getHost(); // Sunucu adını alın
// 	    client->sendReply(RPL_NAMREPLY(client->getPrefix(), channel->getChannelName(), nickList));
//         channel->broadcastMessage(RPL_JOIN(client->getPrefix(), channel->getChannelName()));
//         client->sendReply(RPL_ENDOFNAMES(serverName, client->getNickname(), channel->getChannelName()));
...
// 	    	nickList += *it + " ";
// 	    }
//         std::string serverName = srv->getHost();    
// 	    client->sendReply(RPL_NAMREPLY(client->getPrefix(), channel->getChannelName(), nickList));
//         channel->broadcastMessage(RPL_JOIN(client->getPrefix(), channel->getChannelName()));
//         client->sendReply(RPL_ENDOFNAMES(serverName, client->getNickname(), channel->getChannelName()));
//     }
