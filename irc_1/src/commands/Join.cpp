...
void joinChannel(Client* client, const std::string& channelName, const std::vector<std::string>& commandParts, Server* srv)
{
    ...
    if (srv->hasChannel(channelName))
    {
        ...
        channel->addClient(client);  // Kullanıcıyı kanala ekle
        ...
        channel->broadcastMessage(RPL_JOIN(client->getNickname(), channelName));
        client->sendReply(RPL_ENDOFNAMES(client->getPrefix(), channel->getChannelName()));
